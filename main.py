...
# Capturing the initial frame for the creation of background
while True:
    cv2.waitKey(1000)
    ret, init_frame = cap.read()
    # Check if the frame is returned then break
    if ret:
        init_frame = np.flip(init_frame, axis=1)  # Flip the frame horizontally
        break
...
while True:
    ...
    frame = np.flip(frame, axis=1)  # Flip the frame horizontally
